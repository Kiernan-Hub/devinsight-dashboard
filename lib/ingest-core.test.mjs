...
test("session rows carry ended_at only when the session actually ended", () => {
  const open = sessionRow(validateSession(session).value, { now: NOW });
  assert.equal(open.ended_at, undefined);
  assert.equal(open.last_seen_at, new Date(NOW).toISOString());
  // No denormalised counter: a request only knows its own batch, so an upserted count would
  // overwrite rather than accumulate. session_summary joins the rows for the real number.
  assert.equal(open.sample_count, undefined);
...
// --- gameplay context and events -------------------------------------------

test("gameplay context is optional but range-checked when present", () => {
  const opts = { now: NOW, session: validateSession(session).value };

  const bare = validateSample(sample(), opts);
  assert.equal(bare.value.height, null, "a client that reports no context is still valid");
  assert.equal(bare.value.platform_count, null);

  const rich = validateSample(sample({ height: 4210.5, platform_count: 182, entity_count: 940, lava_speed: 88.2 }), opts);
  assert.equal(rich.value.height, 4210.5);
  assert.equal(rich.value.platform_count, 182);
  assert.equal(rich.value.lava_speed, 88.2);

  assert.equal(validateSample(sample({ platform_count: -4 }), opts).ok, false);
  assert.equal(validateSample(sample({ height: "high" }), opts).ok, false);
});

test("counts are rounded to integers", () => {
  const opts = { now: NOW, session: validateSession(session).value };
  const result = validateSample(sample({ platform_count: 181.6, entity_count: 940.2 }), opts);
  assert.equal(result.value.platform_count, 182);
  assert.equal(result.value.entity_count, 940);
});

test("only known event types are accepted", () => {
  const opts = { now: NOW, session: validateSession(session).value };
  assert.equal(validateEvent({ event_type: "death", height: 4210 }, opts).ok, true);
  // An open vocabulary would let a client invent names no query knows to look for, which the
  // dashboard would then silently ignore. Failing at the door is the louder, better outcome.
  assert.equal(validateEvent({ event_type: "exploded", height: 10 }, opts).ok, false);
  assert.equal(validateEvent({ event_type: "DEATH" }, opts).ok, false);
  assert.equal(validateEvent({}, opts).ok, false);
});

test("events inherit session identity and reject oversized detail", () => {
  const opts = { now: NOW, session: validateSession(session).value };
  const ok = validateEvent({ event_type: "powerup", height: 900, detail: { kind: "shield" } }, opts);
  assert.equal(ok.value.session_id, SESSION_ID);
  assert.equal(ok.value.build_version, "0.5.0");
  assert.deepEqual(ok.value.detail, { kind: "shield" });

  assert.equal(validateEvent({ event_type: "death", detail: "lava" }, opts).ok, false);
  assert.equal(validateEvent({ event_type: "death", detail: { blob: "x".repeat(1200) } }, opts).ok, false);
});

test("events ride along with samples and report their own rejections", () => {
  const result = normalizeIngest({
    session,
    samples: [sample()],
    events: [{ event_type: "death", height: 4210 }, { event_type: "nonsense" }]
  }, { now: NOW });
  ...
  assert.equal(result.rejected.length, 1);
  assert.equal(result.rejected[0].kind, "event");
});

test("events are optional and oversized event batches are refused", () => {
  assert.equal(normalizeIngest({ session, samples: [] }, { now: NOW }).acceptedEvents.length, 0);
  assert.equal(normalizeIngest({ session, samples: [], events: "many" }, { now: NOW }).ok, false);

  const tooMany = normalizeIngest({
    session,
    samples: [],
    events: Array.from({ length: MAX_EVENTS_PER_REQUEST + 1 }, () => ({ event_type: "death" }))
  }, { now: NOW });
