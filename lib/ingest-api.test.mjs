...
test("events are written to their own table, after the session exists", async () => {
  const calls = stubFetch();
  const res = mockRes();
  await handler(mockReq({
    session,
    samples: [sample({ height: 4210, platform_count: 182 })],
    events: [{ event_type: "death", height: 4210, detail: { cause: "lava" } }]
  }), res);

  assert.equal(res.statusCode, 200);
  assert.equal(res.body.accepted, 1);
  assert.equal(res.body.accepted_events, 1);

  assert.equal(calls.length, 3);
  assert.match(calls[0].url, /sessions/);
  assert.match(calls[1].url, /system_logs/);
  assert.match(calls[2].url, /gameplay_events/);

  // Gameplay context rides on the sample row itself, so a correlation query is a plain
  // column comparison rather than a join against a separate timeline.
  assert.equal(calls[1].body[0].height, 4210);
  assert.equal(calls[1].body[0].platform_count, 182);
  assert.deepEqual(calls[2].body[0].detail, { cause: "lava" });
  assert.equal(calls[2].body[0].session_id, SESSION_ID);
});

test("no event table write is attempted when there are no events", async () => {
  const calls = stubFetch();
  const res = mockRes();
  await handler(mockReq({ session, samples: [sample()] }), res);
  assert.equal(calls.length, 2);
  assert.equal(res.body.accepted_events, 0);
});

// Regression test for a fail-open bug. The token check used to be wrapped in
// `if (INGEST_TOKEN) { ... }`, so an unset variable skipped authentication entirely and left
// an unauthenticated endpoint writing with the service-role key. The existing "a bad token is
// rejected" test could never catch it, because it runs with the variable set.
//
// The handler reads its environment once at module load, so this needs a genuinely separate
// module instance rather than a reassignment — hence the cache-busting query string on the
// import specifier, which makes Node treat it as a distinct module.
test("an unconfigured ingest token fails closed rather than open", async () => {
  const saved = process.env.INGEST_TOKEN;
  delete process.env.INGEST_TOKEN;
  try {
    const { default: unconfigured } = await import("../api/ingest.js?no-token");
    const calls = stubFetch();
    const res = mockRes();
    await unconfigured(mockReq({ session, samples: [sample()] }, { token: null }), res);

    assert.equal(res.statusCode, 503, "a missing token must shut the door, not remove it");
    assert.equal(calls.length, 0, "nothing may reach the database without authentication");
  } finally {
    process.env.INGEST_TOKEN = saved;
  }
});

// The body-size limit used to be checked only when req.body arrived as a string. Vercel's Node
// runtime parses an application/json request into an object before the handler runs, so on the
// deployed path that branch never executed and the limit applied to nothing. Checking the
// declared Content-Length covers the parsed-object shape too.
test("an oversized body is refused on the parsed-object path", async () => {
  const calls = stubFetch();
  const res = mockRes();
  const req = mockReq({ session, samples: [sample()] });
  req.headers["content-length"] = String(512 * 1024);

  await handler(req, res);

  assert.equal(res.statusCode, 413);
  assert.equal(calls.length, 0, "an oversized payload must not reach the database");
});

test("a normal Content-Length is unaffected", async () => {
  const calls = stubFetch();
  const res = mockRes();
  const req = mockReq({ session, samples: [sample()] });
  req.headers["content-length"] = "412";
